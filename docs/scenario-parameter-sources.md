...
**Engagement framing.** Interceptor A is a missile-class pursuer.
Target B is a **fighter / attack-aircraft class** target: mass, wing area,
drag, load limit, and speed are grounded together as one airframe class
(ADMIRE-scale fighter with a ~240 m/s cruise example), not a mixed
missile-speed / aircraft-airframe pair.
...
|---|---:|---:|---|
| Initial speed | 600–1,000 m/s → **700 m/s** | 200–300 m/s → **240 m/s** | S: Interceptor [1–4]; Target B [1], [3], [6–7] |
...
scenario **ids** are unchanged; only the names shown in the viewer picker
differ.
...
1. [NASA TM-109057, *The Analysis of a Generic Air-to-Air Missile
   ...
   test altitude, and Mach 0.7 aircraft/target examples (≈240 m/s class
   for Target B speed grounding).
2. [Naval Postgraduate School, *Missile Terminal Guidance and Control
   ...
   1,000 m/s interceptor, 500 m/s missile-class target example,
   6–6.5 km separation, 20 g/9 g limits, and 2 km altitude. Retained as
   an Interceptor A / engagement-geometry anchor; **not** used for
   Target B speed after the fighter-class re-grounding.
3. [*Improvements in Classical Proportional Navigation Guidance Using
   ...
   The 300 m/s target speed marks the upper edge of Target B’s review
   range.
...
6. [FOI-R--1624--SE, *ADMIRE—The Aero-Data Model in a Research
   ...
   Primary airframe anchor for Target B.
7. [AIAA-2024-3194, *Climb Performance of High Thrust-to-Weight Ratio
   ...
   Selected Target B speed **240 m/s** is rounded from this example.
...
- Range and altitude are scenario settings, not intrinsic vehicle
  performance claims.
- Target B is internally consistent as a fighter/attack-aircraft class
  surrogate: speed **240 m/s** (200–300), mass **9,100 kg**, wing
  **45 m²**, Cd **0.035**, Cn_max **1.1**, load limit **9 g**, all
